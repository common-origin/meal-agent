...
Retired as of #35, don't recreate: `.github/PROJECT_STATUS.md` and `.github/README.md` — deleted outright (a hand-maintained status/roadmap and a second doc index, both already stale and duplicating what `PRODUCT.md`/`CLAUDE.md`/GitHub Issues now cover live; git history has them if ever needed). `.github/COPILOT_BUILD.md` (obsolete pre-build bootstrap playbook) and `.github/PHASE_3_CHECKOUT_INTEGRATION.md` (research now tracked as issue #36) — moved to `.github/archive/` rather than deleted, matching this repo's existing convention for point-in-time planning docs (see the other Phase 1/2 docs already there).
