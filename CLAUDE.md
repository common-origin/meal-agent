...
- **Run `/code-review` before merging anything non-trivial, and always check for PR review comments (GitHub Copilot reviews automatically) before merging anything at all.** CI passing means it builds, not that it's correct — Copilot's automatic review alone has caught real bugs multiple times in this repo's history (an actually-broken rate limiter that silently never engaged; false "pure function" and file-location claims written into `ARCHITECTURE.md`/`AI.md` and merged before being checked closely enough). Use a higher `/code-review` effort level (up to `ultra`) for larger or higher-risk changes. This applies to Claude Code sessions too — don't skip checking comments just because CI is green.
